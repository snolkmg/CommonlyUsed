...
public:
    ...
    static QString readFile(QString filePath, QString codecName) {
        QFile file(filePath);
        file.open(QIODevice::ReadOnly | QIODevice::Text);
        QByteArray ba = file.readAll();
        QString allText = QString(codec1ToCodec2(ba, codecName.toLocal8Bit()));
        file.close();
        return allText;
    }
    ...
    //获取文件md5
    static QString getMd5(QString filePath, bool isUpper = true) {
        QFile file(filePath);
...
//        qDebug() << reg;
    ...
    static std::string convertString(QString str) {
    ...
    //写ini
    static void writeVarIni(QString iniPath, QString groupName, QString key, QVariant value)
    {
    ...
    //读ini
    static QVariant readVarIni(QString iniPath, QString groupName, QString key)
    {
        QSettings settings(iniPath, QSettings::IniFormat);
        settings.setIniCodec("utf-8");
        return settings.value(QString("%1/%2").arg(groupName).arg(key));
    }
};
